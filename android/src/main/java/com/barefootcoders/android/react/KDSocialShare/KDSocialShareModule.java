...
public class KDSocialShareModule extends ReactContextBaseJavaModule {
    ...
    public void tweet(ReadableMap options, Callback callback) {
        try {
            ...
            File imageFile = null;
            if (options.hasKey("imagelink")) {
                String shareImageLink = options.getString("imagelink");
                imageFile = new File(shareImageLink);
            }
            
            if (doesPackageExist("com.twitter.android")) {
                try {
                    tweetViaTwitterComposerClass(shareText, imageFile);
                } catch (Exception ex1) {
                    try {
                        tweetViaTwitterDefaultClass(shareText, imageFile);
                    } catch (Exception ex2) {
    ...
    public void shareOnFacebook(ReadableMap options, Callback callback) {
        try {
            ...
            if (options.hasKey("text") && doesPackageExist("com.facebook.katana")) {
                ...
                if (options.hasKey("imagelink")) {
                    File imageFile = new File(options.getString("imagelink"));
                    if (imageFile.exists()) {
    ...
    private void tweetViaTwitterComposerClass(String shareText, File shareImage) throws Exception {
    ...
    private void tweetViaTwitterDefaultClass(String shareText, File shareImage) throws Exception {
        Intent shareIntent = new Intent(Intent.ACTION_SEND);
